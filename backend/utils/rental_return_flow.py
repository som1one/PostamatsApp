...
async def _find_home_return_cell(
    ...
    *,
    ...
    первой. Статус ячейки в БД сразу после выдачи — гонка confirm-pickup,
    вебхуков и reconcile со снапшотами железа: реальный кейс — возврат
    через минуту после забора, когда reconcile успел перезаписать VACANT
    на RESERVED («PIN назначен») или OPENED («дверь открыта»). Поэтому по
    статусу отсекаем только FAULT/DISABLED, а безопасность обеспечивают
    содержательные проверки:
    - в ячейке не числится другой физически присутствующий юнит;
    - на неё не смотрит чужой активный возврат (иначе мы перезапишем
      его PIN на постамате);
    - для не-VACANT статусов: после нашей выдачи в ячейку не оформляли
      никакой возврат (даже проваленный — товар могли успеть положить).
    ...
    if (
        ...
        or cell.status in (LockerCellStatus.FAULT, LockerCellStatus.DISABLED)
    ):
        return None
    ...
    if cell.status != LockerCellStatus.VACANT:
        # «Сталый» не-VACANT статус принимаем, только если после нашей
        # выдачи в эту ячейку не оформляли НИКАКОЙ возврат — даже
        # проваленный. Провал заявки не гарантирует, что товар не положили
        # физически (вебхук закрытия мог потеряться), а выдавать PIN на
        # ячейку с чужим товаром нельзя. Настоящие фантомные статусы
        # (рассинхрон с ESI без каких-либо возвратов) фильтр не задевает.
        later_return = (
            await db.scalars(
                select(ReturnRequest.id)
                .where(
